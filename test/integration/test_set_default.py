...
class SetDefaultTestCase(unittest.TestCase):
    ...
    def _test_default(self, expr=match_redir('1.0/'), expected_message=None,
        ...
        if expected_message:
            ...
            self.assertRegex(
                message,
                r'^Set default version to \S+( in .*)? with dvci \S+$'
            )
...
class TestSetDefault(SetDefaultTestCase):
    ...
    def test_set_default(self):
        self._deploy()
        assertPopen(['dvci', 'set-default', '1.0'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_default()

    def test_custom_template(self):
        self._deploy()
        assertPopen(['dvci', 'set-default', '1.0', '-T',
    ...
    def test_from_subdir(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['dvci', 'set-default', '1.0'], returncode=1)
            assertPopen(['dvci', 'set-default', '1.0', '-F', '../docums.yml'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_default()
    ...
    def test_from_subdir_explicit_origin(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['dvci', 'set-default', '1.0', '-b', 'gh-pages',
                         '-r', 'origin'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_default()
    ...
    def test_branch(self):
        self._deploy('branch')
        assertPopen(['dvci', 'set-default', '1.0', '-b', 'branch'])
        check_call_silent(['git', 'checkout', 'branch'])
        self._test_default()

    def test_commit_message(self):
        self._deploy()
        assertPopen(['dvci', 'set-default', '1.0', '-m', 'commit message'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_default(expected_message='commit message')

    def test_prefix(self):
        self._deploy(prefix='prefix')
        assertPopen(['dvci', 'set-default', '1.0', '--prefix', 'prefix'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_push(self):
        ...
        assertPopen(['dvci', 'set-default', '1.0', '-p'])
    ...
    def test_diverged_remote(self):
        ...
        assertOutput(self, ['dvci', 'set-default', '1.0'], output=(
        ...
        assertPopen(['dvci', 'set-default', '--ignore', '1.0'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), clone_rev)

        assertPopen(['dvci', 'set-default', '--rebase', '1.0'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), origin_rev)
...
class TestSetDefaultOtherRemote(SetDefaultTestCase):
    ...
    def test_explicit_branch(self):
        ...
        assertPopen(['dvci', 'set-default', '1.0', '-p', '-b', 'pages'])
    ...
    def test_explicit_remote(self):
        ...
        assertPopen(['dvci', 'set-default', '1.0', '-p', '-r', 'remote'])
        check_call_silent(['git', 'checkout', 'mybranch'])
        self._test_default()
        self._test_rev('mybranch')
