...
class RetitleTestCase(unittest.TestCase):
    ...
    def _test_retitle(self, expected_message=None, directory='.'):
        ...
        if expected_message:
            ...
            self.assertRegex(
                message,
                r'^Set title of \S+ to 1\.0\.1( in .*)? with dvci \S+$'
            )
...
class TestRetitle(RetitleTestCase):
    ...
    def test_retitle(self):
        self._deploy()
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_retitle()

    def test_branch(self):
        self._deploy('branch')
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-b', 'branch'])
    ...
    def test_from_subdir(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['dvci', 'retitle', '1.0', '1.0.1'], returncode=1)
            assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-F',
                         '../docums.yml'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_retitle()
    ...
    def test_from_subdir_explicit_branch(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-b', 'gh-pages',
                         '-r', 'origin'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_retitle()
    ...
    def test_commit_message(self):
        self._deploy()
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-m',
                     'commit message'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_prefix(self):
        ...
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '--prefix', 'prefix'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_push(self):
        ...
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-p'])
    ...
    def test_diverged_remote(self):
        ...
        assertOutput(self, ['dvci', 'retitle', '1.0', '1.0.1'], output=(
        ...
        assertPopen(['dvci', 'retitle', '--ignore', '1.0', '1.0.1'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), clone_rev)

        assertPopen(['dvci', 'retitle', '--rebase', '1.0', '1.0.1'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), origin_rev)
...
class TestRetitleOtherRemote(RetitleTestCase):
    ...
    def test_explicit_branch(self):
        ...
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-p', '-b', 'pages'])
    ...
    def test_explicit_remote(self):
        ...
        assertPopen(['dvci', 'retitle', '1.0', '1.0.1', '-p', '-r', 'remote'])
        check_call_silent(['git', 'checkout', 'mybranch'])
        self._test_retitle()
        self._test_rev('mybranch')
