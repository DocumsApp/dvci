    ...
    def _test_alias(self, expected_message=None,
        ...
        if expected_message:
            ...
            self.assertRegex(message,
                             r'^Copied \S+ to latest( in .*)? with dvci \S+$')
...
class TestAlias(AliasTestCase):
    ...
    def test_alias(self):
        self._deploy()
        assertPopen(['dvci', 'alias', '1.0', 'latest'])
    ...
    def test_update_aliases(self):
        ...
        assertPopen(['dvci', 'deploy', '2.0'])
        assertPopen(['dvci', 'alias', '2.0', 'latest', '-u'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_alias_copy(self):
        self._deploy()
        assertPopen(['dvci', 'alias', '1.0', 'latest', '--no-redirect'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_alias(redirect=False)

    def test_aliases_custom_template(self):
        self._deploy()
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-T',
    ...
    def test_from_subdir(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['dvci', 'alias', '1.0', 'latest'], returncode=1)
            assertPopen(['dvci', 'alias', '1.0', 'latest', '-F',
                         '../docums.yml'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_alias()
    ...
    def test_branch(self):
        self._deploy('branch')
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-b', 'branch'])
        check_call_silent(['git', 'checkout', 'branch'])
        self._test_alias()

    def test_commit_message(self):
        self._deploy()
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-m',
                     'commit message'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_prefix(self):
        ...
        assertPopen(['dvci', 'alias', '1.0', 'latest', '--prefix', 'prefix'])
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_push(self):
        ...
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-p'])
    ...
    def test_diverged_remote(self):
        ...
        assertOutput(self, ['dvci', 'alias', '1.0', 'latest'], output=(
        ...
        assertPopen(['dvci', 'alias', '--ignore', '1.0', 'latest'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), clone_rev)

        assertPopen(['dvci', 'alias', '--rebase', '1.0', 'latest'])
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), origin_rev)
...
class TestAliasOtherRemote(AliasTestCase):
    ...
    def test_explicit_branch(self):
        ...
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-p', '-b', 'pages'])
    ...
    def test_explicit_remote(self):
        ...
        assertPopen(['dvci', 'alias', '1.0', 'latest', '-p', '-r', 'remote'])
        check_call_silent(['git', 'checkout', 'mybranch'])
        self._test_alias()
        self._test_rev('mybranch')
